...
* UI waits for completely-hidden movement

h3. Organization
* probably should have a controller for InfoBarView
* BATAnimation isn't really BAT, since its implementations use Views

h3. API
* HMPathFinder doesn't need "minCost" parameter, since it can be inferred from the "map" parameter
* BATGame (and BRGame?) needs a "startGame" method (and can't use "initialize"!) so can restart after ending game
