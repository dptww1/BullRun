...
** add convenience methods for adding/removing reinforcements which are then called by the control-specific callbacks
** create outlets and wire everything up: Bartow, Bee, Jackson, Smith
...
* HMPathFinder needs to pass path (or something) to callback; can use that info to prevent traffic jamsh
